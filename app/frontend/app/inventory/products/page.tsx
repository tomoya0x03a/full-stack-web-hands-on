                ...
                <tbody>
                    ...
                    {data.map((data: any) => (
                        editingRow === data.id ? (
                            <tr key={data.id}>
                            <td>{data.id}</td>
                            <td><input type="text" value={input.name} onChange={handleInput} /></td>
                            <td><input type="number" value={input.price} onChange={handleInput} /></td>
                            <td><input type="text" value={input.description} onChange={handleInput} /></td>
                            <td></td>
                            <td>
                                <button onClick={(event) => handleEditCancel(event)}>キャンセル</button>
                                <button onClick={(event) => handleEdit(event)}>更新する</button>
                                <button onClick={() => handleEditCancel(data.id)}>削除する</button>
                            </td>
                            </tr>                            
                        ) : (
                            <tr key={data.id}>
